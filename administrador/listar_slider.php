...
$p = $p+0;
$in = $p*$max;
$total = @mysql_num_rows(mysql_query("SELECT * FROM {$nomeDaPagina} WHERE tipo='{$tipo}' ".($buscar<>'' ? " AND (titulo LIKE '%$buscar%')" : "")." ORDER BY codigo DESC"));
$pagina = (ceil($total/$max)-1);
$query = mysql_query("SELECT * FROM {$nomeDaPagina} WHERE tipo='{$tipo}' ".($buscar<>'' ? " AND (titulo LIKE '%$buscar%')" : "")." ORDER BY codigo DESC LIMIT $in,$max");
		...
		<div class="listagem-table">
			<?php if($totalItens>0) { 
				while($row = mysql_fetch_object($query)) {
					$empreendimento = mysql_fetch_object(mysql_query("SELECT titulo FROM empreendimentos WHERE codigo='{$row->cod_empreendimento}' ORDER BY titulo ASC")) ?>
					<article class="col-md-3 col-sm-4">
						<div class="listagem-item">
							...
							<p><strong>Aparecer somente em: <?php echo $App->SubStrTexto($empreendimento->titulo, 50); ?></strong></p>
							<?php if($row->dataAtiva=="S") { ?>
								<p><?php echo $App->Data($row->dataInicio)." à ".$App->Data($row->dataFinal); ?></p>
							<?php } ?>
