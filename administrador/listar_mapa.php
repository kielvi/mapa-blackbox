...
$p = $p+0;
$in = $p*$max;
$total = @mysql_num_rows(mysql_query("SELECT * FROM {$nomeDaPagina} WHERE 1=1 ".($buscar<>'' ? " AND (empreendimento LIKE '%$buscar%')" : "")." ORDER BY codigo DESC"));
$pagina = (ceil($total/$max)-1);
$query = mysql_query("SELECT * FROM {$nomeDaPagina} WHERE 1=1 ".($buscar<>'' ? " AND (empreendimento LIKE '%$buscar%')" : "")." ORDER BY codigo DESC LIMIT $in,$max");
...
<div class="content listagem">
	...
	<div class="container-fluid">
		...
		<div class="col-md-5 listagem-buttons">
			<a href="<?php echo "adicionar_$nomeDaPagina.php?acao=adicionar&nomeDaPagina=$nomeDaPagina&tabela=$tabela&tipo=$tipo"; ?>" class="<?php echo ($buttonAdd==FALSE ? "hidden-xs-up" : ""); ?>"><span>ADICIONAR NOVO</span></a>
		...
		<div class="listagem-table">
			<?php if($totalItens>0) { 
				while($row = mysql_fetch_object($query)) {
					$categoria = mysql_fetch_object(mysql_query("SELECT categoria FROM empreendimentos_categorias WHERE codigo='$row->categoria'")); ?>
					<article class="col-md-3 col-sm-4">
						<div class="listagem-item">
							<figure><a href="<?php echo ($row->pin<>'' ? "../".$row->pin : "../assets/images/gmaps-marker.png"); ?>" data-toggle="lightbox" style="background:url(<?php echo ($row->pin<>'' ? "../".$row->pin : "../assets/images/gmaps-marker.png"); ?>) no-repeat center;"> </a></figure>
							<h1><?php echo $App->SubStrTexto($row->titulo, 50); ?></h1>
							<p><?php echo $App->SubStrTexto($row->subtitulo, 50); ?></p>

							<div class="col-xs-12 listagem-item-buttons" align="center">
								<a class="<?php echo ($buttonEdit==FALSE ? "hidden-xs-up" : ""); ?>" href="<?php echo "adicionar_$nomeDaPagina.php?codigo=$row->codigo&acao=editar&nomeDaPagina=$nomeDaPagina&tabela=$tabela&tipo=$tipo"; ?>" target="paginaTarget"><i class="fa fa-fw fa-pencil-square-o" aria-hidden="true"></i><span>Editar</span></a>
								<a class="<?php echo ($buttonFotos==FALSE ? "hidden-xs-up" : ""); ?>" href="<?php echo "maisfotos_$nomeDaPagina.php?codigo=$row->codigo&nomeDaPagina=$nomeDaPagina&tabela=$tabela"; ?>"><i class="fa fa-fw fa-picture-o" aria-hidden="true"></i><span>Mais fotos</span></a>
								<a class="<?php echo ($buttonDel==FALSE ? "hidden-xs-up" : ""); ?>" href="javascript:void(0)" data-href="<?php echo $urlPadrao; ?>&codigo=<?php echo $row->codigo; ?>&acao=deletar&arquivoDelete=<?php echo "../".$row->foto; ?>&tipo=<?php echo $tipo; ?>" data-toggle="modal" data-target="#confirmar-delete"><i class="fa fa-fw fa-trash-o"></i><span>Apagar</span></a>
							</div>
						</div>
					</article>
			<?php } 
		...
		<div class="listagem-pag <?php echo ($total<=$max ? "hidden-xs-up" : ""); ?>">
			<?php $menos=$p-1;
			$mais= $p+1;
