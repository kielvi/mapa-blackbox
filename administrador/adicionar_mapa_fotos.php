		...
		<div class="container adicionar">
			<form id="form-adicionar" name="form-adicionar" method="post" action="" enctype="multipart/form-data">
				...
				<div class="row">
					...
					<div class="col-md-7 form-group">
						<label for="cod_mapa">Empreendimento</label>
						<select id="cod_mapa" name="cod_mapa" class="form-control" required>
							<option>Selecione</option>
							<?php $query_empreendimentos = mysql_query("SELECT * FROM mapa ORDER BY titulo ASC"); 
							while($row_empreendimentos = mysql_fetch_object($query_empreendimentos)) { ?>
								<option value="<?php echo $row_empreendimentos->codigo; ?>" <?php echo ($row->cod_mapa==$row_empreendimentos->codigo ? "selected" : false); ?>><?php echo $row_empreendimentos->titulo; ?></option>
							<?php } ?>
						</select>
					</div>
...
if(isset($acaoForm)) {
	...
	if($acaoForm=="adicionar") {
		mysql_query("INSERT INTO $nomeDaPagina (cod_mapa, ordem, legenda) VALUES ('$cod_mapa', '$ordem', '$legenda')");
	...
	if($acaoForm=="editar") { 
		...
		mysql_query("UPDATE $nomeDaPagina SET cod_mapa='$cod_mapa', ordem='$ordem', legenda='$legenda' WHERE codigo='$codigo'");
