...
$p = $p+0;
$in = $p*$max;
$total = @mysql_num_rows(mysql_query("SELECT * FROM {$nomeDaPagina} WHERE 1=1 ".($buscar<>'' ? " AND (categoria LIKE '%$buscar%')" : "")." ORDER BY codigo DESC"));
$pagina = (ceil($total/$max)-1);
$query = mysql_query("SELECT * FROM {$nomeDaPagina} WHERE 1=1 ".($buscar<>'' ? " AND (categoria LIKE '%$buscar%')" : "")." ORDER BY codigo DESC LIMIT $in,$max");
					...
					<table class="table table-bordered table-striped table-hover">
						<thead class="thead-default">
							<tr>
								...
								<th>Lazer e convivência</th>
								<th>Ações</th>
							</tr>
						</thead>
						<tbody>
							<?php while($row = mysql_fetch_object($query)) {
								$empreendimento = mysql_fetch_object(mysql_query("SELECT titulo FROM empreendimentos WHERE codigo='{$row->empreendimento}' ORDER BY titulo ASC")) ?>
								<tr>
									<th scope="row"><?php echo $row->codigo; ?></th>
									<td><?php echo $App->SubStrTexto($empreendimento->titulo, 50); ?></td>
									<td><?php echo $App->SubStrTexto($row->lazer, 50); ?></td>
									<td>
										<div class="listagem-item-buttons" align="center">
...
(function() {
	...
	[].slice.call( document.querySelectorAll( '.table tr td a' ) ).forEach( function( el ) {
