...
$p = $p+0;
$in = $p*$max;
$total = @mysql_num_rows(mysql_query("SELECT * FROM {$nomeDaPagina} WHERE tipo='{$tipo}' ".($buscar<>'' ? " AND (nome LIKE '%$buscar%' OR email LIKE '%$buscar%')" : "")." ORDER BY codigo DESC"));
$pagina = (ceil($total/$max)-1);
$query = mysql_query("SELECT * FROM {$nomeDaPagina} WHERE tipo='{$tipo}' ".($buscar<>'' ? " AND (nome LIKE '%$buscar%' OR email LIKE '%$buscar%')" : "")." ORDER BY codigo DESC LIMIT $in,$max");
						...
						<tbody>
							<?php while($row = mysql_fetch_object($query)) { ?>
								<tr>
									...
									<td><?php echo $App->SubStrTexto($row->nome, 50); ?></td>
									<td><?php echo $App->SubStrTexto($row->email, 50); ?></td>
									<td>
