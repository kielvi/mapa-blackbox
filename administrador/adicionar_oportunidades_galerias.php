		...
		<div class="container adicionar">
			<form id="form-adicionar" name="form-adicionar" method="post" action="" enctype="multipart/form-data">
				...
				<div class="row">
					...
					<div class="col-md-7 form-group">
						<label for="cod_oportunidade">Oportunidade</label>
						<select id="cod_oportunidade" name="cod_oportunidade" class="form-control" required>
							<option>Selecione</option>
							<?php $query_oportunidades = mysql_query("SELECT * FROM oportunidades ORDER BY titulo ASC"); 
							while($row_oportunidades = mysql_fetch_object($query_oportunidades)) { ?>
								<option value="<?php echo $row_oportunidades->codigo; ?>" <?php echo ($row->cod_oportunidade==$row_oportunidades->codigo ? "selected" : false); ?>><?php echo $row_oportunidades->titulo; ?></option>
							<?php } ?>
						</select>
					</div>
...
if(isset($acaoForm)) {
	...
	if($acaoForm=="adicionar") {
		mysql_query("INSERT INTO $nomeDaPagina (cod_oportunidade, ordem, legenda) VALUES ('$cod_oportunidade', '$ordem', '$legenda')");
	...
	if($acaoForm=="editar") { 
		...
		mysql_query("UPDATE $nomeDaPagina SET cod_oportunidade='$cod_oportunidade', ordem='$ordem', legenda='$legenda' WHERE codigo='$codigo'");
