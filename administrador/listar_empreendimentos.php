...
$p = $p+0;
$in = $p*$max;
$total = @mysql_num_rows(mysql_query("SELECT * FROM {$nomeDaPagina} WHERE 1=1 ".($buscar<>'' ? " AND (titulo LIKE '%$buscar%')" : "")." ORDER BY codigo DESC"));
$pagina = (ceil($total/$max)-1);
$query = mysql_query("SELECT * FROM {$nomeDaPagina} WHERE 1=1 ".($buscar<>'' ? " AND (titulo LIKE '%$buscar%')" : "")." ORDER BY codigo DESC LIMIT $in,$max");
				...
				while($row = mysql_fetch_object($query)) {
					...
					<article class="col-md-3 col-sm-4">
						<div class="listagem-item">
							<figure><a href="<?php echo ($row->fachada<>'' ? "../".$row->fachada : "images/nophoto.svg"); ?>" data-toggle="lightbox" style="background-image:url(<?php echo ($row->fachada<>'' ? "../".$row->fachada : "images/nophoto.svg"); ?>);"> </a></figure>
