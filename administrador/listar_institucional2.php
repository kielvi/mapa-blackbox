...
$p = $p+0;
$in = $p*$max;
$total = @mysql_num_rows(mysql_query("SELECT * FROM {$nomeDaPagina} WHERE tipo='{$tipo}' ".($buscar<>'' ? " AND (titulo LIKE '%$buscar%' OR referencia LIKE '%$buscar%' OR descricao LIKE '%$buscar%')" : "")." ORDER BY codigo DESC"));
$pagina = (ceil($total/$max)-1);
$query = mysql_query("SELECT * FROM {$nomeDaPagina} WHERE tipo='{$tipo}' ".($buscar<>'' ? " AND (titulo LIKE '%$buscar%' OR referencia LIKE '%$buscar%' OR descricao LIKE '%$buscar%')" : "")." ORDER BY codigo DESC LIMIT $in,$max");
